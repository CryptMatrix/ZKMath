...
public:
	...
	uint32_t np_pt, np_rg, np_sz;  // 什么参数？
	uint32_t rand_pt;  // 什么参数？

	uint32_t edabit_num, edabit_offset;

	// const uint32_t N = 800000, B = 2, C = 2; // B=2, C=2, N>=741455
						 // B=3, C=3, N>=6251
	const uint32_t N = 10000, B = 3, C = 3;

	uint32_t ell; // ell = NB+C
	...
	void edabits_gen_backend()
	{
		...
		if (party == ALICE)
			...
			auth_helper->open_check_recv(bool_candidate + N + rand_pt, arith_candidate + np_pt + N + rand_pt, C);
	...
	{
		int counter = 0;
		int round1_num, round, leftover; // leftover 剩下的。round是什么？
		if (len <= edabit_num)
		{
			round = 1;
			round1_num = len;
	...
	void next_edabits(uint32_t &f2_indexin, uint32_t &fp_index)
	{
		if (edabit_num == 0)
		{
			np_pt = np_rg;
			edabits_gen_backend();
			std::cout << "Party id: " << party << ", edabits gen invoke! " << std::endl;
		}
